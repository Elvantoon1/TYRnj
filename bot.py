...
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8136598267:AAH37_t_ITrREbkU-h-tD4MBLrMMcXNYGNk")
ADMIN_ID = os.environ.get("ADMIN_ID", "7237912884")
...
class CacheManager:
    ...
    def get_countries(self) -> List[Dict]:
        ...
        if not self.countries_cache or self._is_expired(
            max(item.get('cache_time', 0) for item in self.countries_cache.values()) if self.countries_cache else 0,
    ...
    def get_country_counts(self, country_id: int) -> Dict:
        ...
        if country_id not in self.country_counts_cache or self._is_expired(
            ...
            conn = self.db_connect()
            if conn is None:
                return {'total_count': 0, 'premium_count': 0, 'cache_time': time.time()}
            cur = conn.cursor()
            try:
                cur.execute("""
                    SELECT 
                ...
                self.country_counts_cache[country_id] = {
                    ...
                    'premium_count': row[1] if row else 0,
                    'cache_time': time.time()
                }
            except Exception as e:
    ...
    try:
        ...
        for index_sql in indexes:
            try:
                ...
                logger.warning(f"خطأ في إنشاء الفهرس {index_sql}: {e}")
...
def bulk_import_numbers(country_id: int, numbers_iterable, batch_size: int = 5000) -> Dict[str, Any]:
    ...
    conn = db_connect()
    if conn is None:
        return {'processed': 0, 'inserted': 0, 'skipped': 0, 'errors': 1, 'errors_list': ['خطأ في الاتصال بقاعدة البيانات']}
...
def get_country_by_id(country_id: int) -> Optional[Dict]:
    ...
    try:
        ...
        logger.error(f"خطأ في جلب الدولة {country_id}: {e}")
        return None
    finally:
        conn.close()
            ...
            if row and row[0]:
                ...
                if row[1]:
                    try:
                        ...
                        # إذا كان تنسيق التاريخ غير صحيح
                        is_pro = True
...
def get_premium_numbers(country_id: int, premium_type: Optional[str] = None) -> List[Dict]:
    ...
    try:
        if premium_type:
            ...
            for num in numbers:
                num_dict = dict(num)
                num_type = get_premium_pattern_type(num_dict['number'])
                if num_type == premium_type:
                    filtered_numbers.append(num_dict)
    ...
    for channel in required_channels:
        try:
            if channel.startswith('-'):
                # قناة مجموعة
                chat_member = bot.get_chat_member(int(channel), user_id)
            else:
                # قناة عادية
                channel_clean = channel.lstrip('@')
                chat_member = bot.get_chat_member(channel_clean, user_id)
    ...
    try:
        ...
        for user_id in users:
            try:
                ...
                # إرسال الرسالة
                if safe_send(user_id, f"<b>{ad[1]}</b>\n\n{ad[2]}"):
                    sent_count += 1
                else:
                    failed_count += 1
                    errors_list.append(f"فشل إرسال للمستخدم {user_id}")
                
                # تحديث التقدم
                cur.execute("""
                    UPDATE broadcast_progress 
                    SET sent_count = ?, failed_count = ?, current_user_id = ?, errors = ?
                    WHERE broadcast_id = ?
                """, (sent_count, failed_count, user_id, '\n'.join(errors_list[-10:]), broadcast_id))
                ...
                failed_count += 1
                errors_list.append(f"خطأ في إرسال الإذاعة للمستخدم {user_id}: {str(e)}")
...
def set_invited_by(user_id: int, inviter_id: int):
    ...
    cur = conn.cursor()
    try:
        cur.execute("UPDATE users SET invited_by = ? WHERE id = ?", (inviter_id, user_id))
        cur.execute("UPDATE users SET total_invites = total_invites + 1 WHERE id = ?", (inviter_id,))
        conn.commit()
...
✅ بعد الاشتراك، اضغط على زر "تحقق من الاشتراك".
    ...
    if not country:
        bot.answer_callback_query(cq.id, "❌ الدولة غير موجودة!", show_alert=True)
        return
...
• 🔒 النقاط تمنح بعد انضمام المدعو للقنوات المطلوبة
        ...
        if invited_users:
            ...
            for i, user in enumerate(invited_users, 1):
                username = user.get('username') or user.get('first_name') or 'مجهول'
                text += f"{i}. {username}\n"
...
أعد إرسال الرمز الصحيح:""")
    ...
    conn = db_connect()
    if conn is None:
        safe_send(uid, "❌ <b>خطأ في الاتصال بقاعدة البيانات!</b>")
        AWAITING_PROOF.pop(uid, None)
        return
...
def handle_number_pattern(message):
    ...
    country = get_country_by_id(country_id)
    if not country:
        safe_send(uid, "❌ <b>الدولة غير موجودة!</b>")
        AWAITING_NUMBER_PATTERN.pop(uid, None)
        return
...
def cb_admin_panel(cq):
    ...
    conn = db_connect()
    if conn is None:
        bot.answer_callback_query(cq.id, "❌ خطأ في الاتصال بقاعدة البيانات!")
        return
...
def cb_admin_stats(cq):
    ...
    try:
        ...
        stats['proofs_24h'] = cur.fetchone()[0]
        
        cur.execute("SELECT COUNT(*) as count FROM proofs WHERE verified = 1")
        stats['verified_proofs'] = cur.fetchone()[0]
        
        cur.execute("SELECT COUNT(*) as count FROM numbers")
        stats['total_numbers'] = cur.fetchone()[0]
        
        cur.execute("SELECT COUNT(*) as count FROM numbers WHERE is_premium = 1")
        stats['premium_numbers'] = cur.fetchone()[0]
        
        cur.execute("SELECT COUNT(*) as count FROM countries WHERE is_active = 1")
        stats['active_countries'] = cur.fetchone()[0]
        
        cur.execute("SELECT SUM(points) as total FROM users")
        stats['total_points'] = cur.fetchone()[0] or 0
        
        cur.execute("SELECT SUM(points) as total FROM points_history WHERE points > 0")
        stats['points_distributed'] = cur.fetchone()[0] or 0
        
        # الدول الأكثر نشاطاً
        cur.execute("""
            SELECT c.name, COUNT(p.id) as proof_count 
            FROM proofs p 
            JOIN countries c ON p.country_name = c.name 
        ...
        text = f"""📊 <b>الإحصائيات الشاملة</b>

👥 <b>المستخدمين:</b>
• 📈 المستخدمين النشطين: {stats['active_users']}
• 🚫 المستخدمين المحظورين: {stats['banned_users']}
• ⭐ مشتركي PRO: {stats['pro_users']}

📞 <b>الأرقام:</b>
• 📊 إجمالي الأرقام: {stats['total_numbers']}
• 💎 الأرقام المميزة: {stats['premium_numbers']}
• 🌍 الدول النشطة: {stats['active_countries']}

✅ <b>الإثباتات:</b>
• ✅ الإثباتات المؤكدة: {stats['verified_proofs']}
• 📅 إثباتات الـ24 ساعة: {stats['proofs_24h']}

🪙 <b>النقاط:</b>
• 💰 إجمالي النقاط: {stats['total_points']}
• 📤 نقاط موزعة: {stats['points_distributed']}
        """
        
        if top_countries:
            text += "\n🏆 <b>أكثر الدول نشاطاً (أسبوع):</b>\n"
            for i, (country, count) in enumerate(top_countries, 1):
                text += f"{i}. {country}: {count} إثبات\n"
...
def get_invited_users(user_id: int) -> List[Dict]:
    ...
    cur = conn.cursor()
    try:
        cur.execute("SELECT id, username, first_name FROM users WHERE invited_by = ?", (user_id,))
        rows = cur.fetchall()
        return [dict(row) for row in rows]
    except Exception as e:
        logger.error(f"خطأ في جلب المدعوين: {e}")
        return []
    finally:
        conn.close()
...
def cleanup_old_data():
    ...
    try:
        ...
        cutoff_date = (datetime.now() - timedelta(days=auto_cleanup_days)).strftime('%Y-%m-%d %H:%M:%S')
        ...
        cur.execute("DELETE FROM logs WHERE created_at < ?", (cutoff_date,))
        logs_deleted = cur.rowcount
        
        # تنظيف نقاط التاريخ القديم
        cur.execute("DELETE FROM points_history WHERE created_at < ?", (cutoff_date,))
        points_deleted = cur.rowcount
        
        # تنظيف أنماط البحث القديمة
        cur.execute("DELETE FROM number_patterns WHERE searched_at < ?", (cutoff_date,))
        patterns_deleted = cur.rowcount
        
        conn.commit()
        conn.close()
        
        if any([logs_deleted, points_deleted, patterns_deleted]):
            logger.info(f"🧹 تم تنظيف البيانات القديمة: {logs_deleted} سجل، {points_deleted} نقاط، {patterns_deleted} نمط")
...
def cb_admin_help(cq):
    ...
    markup = admin_back_keyboard()
    safe_edit_message(ADMIN_HELP_TEXT, cq.message.chat.id, cq.message.message_id, markup)
    bot.answer_callback_query(cq.id)

# ================================
# إدارة حالات المستخدمين المؤقتة
# ================================

def cleanup_user_states():
    """تنظيف الحالات المؤقتة للمستخدمين"""
    now = time.time()
    expired_users = []
    
    # تنظيف حالة التصفح
    for uid, state in BROWSE.items():
        if now - state.get("timestamp", 0) > 3600:  # ساعة واحدة
    ...
    # تنظيف حالات الإثبات
    expired_proofs = [uid for uid, data in AWAITING_PROOF.items() if now - data.get("timestamp", 0) > 1800]  # 30 دقيقة
    for uid in expired_proofs:
        AWAITING_PROOF.pop(uid, None)
    
    # تنظيف حالات البحث
    expired_patterns = [uid for uid, data in AWAITING_NUMBER_PATTERN.items() if now - data.get("timestamp", 0) > 900]  # 15 دقيقة
    for uid in expired_patterns:
        AWAITING_NUMBER_PATTERN.pop(uid, None)
    
    # تنظيف حالات الأرقام المميزة
    expired_premium = [uid for uid, data in AWAITING_PREMIUM_FILTER.items() if now - data.get("timestamp", 0) > 1800]  # 30 دقيقة
    for uid in expired_premium:
        AWAITING_PREMIUM_FILTER.pop(uid, None)
    
    if any([expired_users, expired_proofs, expired_patterns, expired_premium]):
        logger.info(f"🧹 تم تنظيف الحالات المؤقتة: {len(expired_users)} تصفح، {len(expired_proofs)} إثبات، {len(expired_patterns)} بحث، {len(expired_premium)} أرقام مميزة")

def user_states_cleanup_worker():
    """خيط عمل تنظيف الحالات المؤقتة"""
    while True:
        try:
            cleanup_user_states()
            time.sleep(300)  # كل 5 دقائق
        except Exception as e:
            logger.error(f"❌ خطأ في خيط تنظيف الحالات: {e}")
            time.sleep(300)
...
def main():
    ...
    try:
        logger.info("🚀 بدء تشغيل بوت أرقام مجانية...")
        
        # تهيئة قاعدة البيانات
        init_db()
        
        # بدء خيوط العمل
        pro_worker_thread = threading.Thread(target=pro_expiry_worker, daemon=True)
        cleanup_worker_thread = threading.Thread(target=cleanup_worker, daemon=True)
        user_states_worker_thread = threading.Thread(target=user_states_cleanup_worker, daemon=True)
        
        pro_worker_thread.start()
        cleanup_worker_thread.start()
        user_states_worker_thread.start()
        
        logger.info("✅ تم بدء خيوط العمل بنجاح")
